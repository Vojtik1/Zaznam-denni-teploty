...
app.post('/savedata', urlencodedParser, (req, res) => {
    ...
    fs.appendFile(path.join(__dirname, 'teploty.csv'), str, function (err) {
...
app.get("/todolist", (req, res) => {
    ...
    csvtojson({headers:['stupne','den','hodina']}).fromFile(path.join(__dirname, 'teploty.csv'))
    .then(data => {
        /* Vypsání získaných dat ve formátu JSON do konzole */
        console.log(data);
        /* Možnost seřazení dat podle sloupce odevzdani - vzestupně */
        data.sort(function(a, b) { 
            if (a.hodina > b.hodina) return 1
            else if (a.hodina < b.hodina) return -1
            else return 0; 
        })
        /* Vykreslení šablony index.pug i s předanými daty (objekt v druhém parametru) */
        res.render('index', {nadpis: "Seznam stupňů", den: data});
    })
    .catch(err => {
